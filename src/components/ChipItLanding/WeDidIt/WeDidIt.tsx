...
export const WeDidIt: React.FC = () => {
  ...
  return (
    <WeDidItContent
      ...
      cardData={[
        {
          title: t(data.cardBusiness.title),
          sentences: data.cardBusiness.sentences.map((sentence) => t(sentence)),
        },
        {
          title: t(data.cardClient.title),
          sentences: data.cardClient.sentences.map((sentence) => t(sentence)),
        },
      ]}
    />
  );
